...
void set_sysclk_pll(void)
{
	...
	//f_PLL_out = 168
	//
	//WRITE_BITS(addr, highest_bit, lowest_bit, data)

	WRITE_BITS(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLP_1_BIT, PLLP_0_BIT, 0b00); //Set P
	WRITE_BITS(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLN_8_BIT, PLLN_0_BIT, 84);  //Set N=84
	WRITE_BITS(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLM_5_BIT, PLLM_0_BIT, 4);  //Set M=4
	...
	while (READ_BIT(RCC_BASE + RCC_CFGR_OFFSET, SW_1_BIT) != 1)
		;
	while (READ_BIT(RCC_BASE + RCC_CFGR_OFFSET, SW_0_BIT) != 0)
		;
}
